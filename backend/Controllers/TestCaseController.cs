...
namespace backend.Controllers
{
    ...
    public class TestCaseController : ControllerBase
    {
        ...
        /// <returns>A paginated result of test cases.</returns>
        /// <response code="200">Returns the paginated list of test cases.</response>
        [HttpGet("paged")]
        public async Task<ActionResult<PagedResult<TestCaseDto>>> GetPaged(
            ...
            [FromQuery] bool sortDescending = false)
        {
            try
            {
                ...
                var result = await _testCaseService.GetPagedAsync(parameters);
                return Ok(result);
            }
            catch (Exception ex)
            {
